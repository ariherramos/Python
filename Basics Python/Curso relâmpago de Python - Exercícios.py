...
# enquanto o dicionário possui chave e valor
...
def encontreCachorro(texto):
    print('cachorro' in texto)
    return True if texto.find('cachorro') >0 else False
...
def contaCachorro(texto):
    list = texto.lower().strip('.').split()
    num = 0
    for i in list:
        if i == 'cachorro':
            num = num + 1
